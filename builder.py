...
if __name__ == '__main__':
    ...
    track_A = section(track_base, thick, [0, altitude - thick/2], matboard)
    track_B = section(track_base, thick, [0, altitude - (3/2)*thick], matboard)
    web_L = section(thick, altitude - 3*thick, [-((web_sep/2) + (thick/2)), (altitude-3*thick)/2], matboard)
    web_R = section(thick, altitude - 3*thick, [((web_sep/2) + thick/2), (altitude-3*thick)/2], matboard)
    fold_L = section(fold, thick, [-(web_sep/2 + fold/2), altitude - (5/2)*thick], matboard)
    fold_R = section(fold, thick, [(web_sep/2 + fold/2), altitude - (5/2)*thick], matboard)

    sections = [track_A, track_B, web_L, web_R, fold_L, fold_R]         # temp storage soltuin
